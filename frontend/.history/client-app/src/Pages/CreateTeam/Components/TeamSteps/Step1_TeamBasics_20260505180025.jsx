...
function Step1_TeamBasics({ formData, setFormData, onNext, hackathonList }) {
  ...
  // 1. Add a local state at the top of your component
const [isOtherActive, setIsOtherActive] = useState(false);

// 2. Update the Size Click handler
const handleSizeClick = (size) => {
  setIsOtherActive(false); // Reset "Other" if a standard number is picked
  setFormData({ ...formData, teamSize: size });
};

// 3. Handle the "Other" button click
const handleOtherClick = () => {
  setIsOtherActive(true);
  setFormData({ ...formData, teamSize: '' }); // Clear to let user type
};

// 4. Handle custom number typing
        ...
        <div className={styles.sizeContainer}>
          {[2, 3, 4, 5, 6].map(size => (
            <button
              ...
              className={`${styles.sizeBox} ${formData.teamSize === size ? styles.sizeBoxActive : ''}`}
            >
              {size}
            </button>
          ))}
