...
function Step3_RequiredSkills({
  ...
  const handleAddValue = (field, newValue) => {
    if (!newValue|| ! newValue.value || newValue.value.toString().trim()==="" || formData[field].includes(newValue.value)) return;
    setFormData({
      ...formData,
      [field]: [...formData[field], newValue.value]
    });
  };
  const handleAddSkill = (newValue) => {
    if (!newValue || ! newValue.value || newValue.value.toString().trim()==="") return;
