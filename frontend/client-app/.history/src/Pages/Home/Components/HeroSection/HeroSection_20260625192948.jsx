import styles from "./HeroSection.module.css";
import { Container, Row, Col } from "react-bootstrap";
import HeroImg from "../../../../assets/landing1.png";
import { Sparkles } from "lucide-react";
import CustomButton from "../../../../shared/CustomButton/CustomButton";
import { useNavigate } from "react-router-dom";

function HeroSection() {
  const navigate = useNavigate()
  const word = "TeamCatalyst";
  return (
    <section className="py-5">
      <Container className="py-5">
        <Row className="gy-5">
          <Col xs={12} lg={6} className="d-flex flex-column align-items-center align-items-lg-start">
            <div
              className={`d-inline-flex align-items-center gap-2 px-5 py-2 rounded-pill mb-4  ${styles.badgeWrapper}  ${styles.badge}`}
            >
              <Sparkles size={20} />
              <span className="text-sm-center d-block w-100 ">AI-powered Platform</span>
            </div>

            <h1 className={`text-center text-lg-start fw-bold mb-4  ${styles.heroTitle}`}>
              Elevate your hackathon game with{" "}
              <span className={styles.textGradient}>TeamCatalyst</span>
              {/* <span className={`${styles.loader} ${styles.textGradient}`}>
                {word.split("").map((letter, index) => (
                  <span key={index} style={{ "--i": index + 1 }}>
                    {letter}
                  </span>
                ))}
              </span> */}
            </h1>
            <p className={`${styles.heroSubTitle} text-center text-lg-start fw-medium`}>
              Build the perfect hackathon team using AI-powered skill and
              personality matching.
            </p>
            <Row className="mt-3 w-100 gx-3 gy-3">

              <Col xs={12} md={6}>
                <CustomButton variant="primary" className="w-100 h-100" onClick={() => navigate('/create-Team')}>
                  Create a team
                </CustomButton>
              </Col>

              <Col xs={12} md={6}>
                <CustomButton variant="secondary" className="w-100 h-100" onClick={() => navigate('/explore-Hackathons')}>
                  Explore Hackathons
                </CustomButton>
              </Col>

            </Row>
          </Col>

          <Col xs={12} lg={6} className="ms-auto d-none d-lg-block ">
            <img src={HeroImg} alt="Hero Image" className={styles.heroImg} />
          </Col>
        </Row>
      </Container>
    </section>
  );
}

export default HeroSection;
