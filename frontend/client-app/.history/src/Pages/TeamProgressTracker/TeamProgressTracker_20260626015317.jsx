...
function TeamProgressTracker() {
 const [modalConfig, setModalConfig] = useState({ 
  ...
  // 2. Turn teamData into an array of teams
  const [teams, setTeams] = useState([
    {
      id: 101, // Added team ID
    ...
    },
    {
      ...
      members: [
        { id: 4, name: "Alex Turner", linkedin: "#", github: "#", status: "ACCEPT" },
        { id: 5, name: "Maria Garcia", linkedin: "#", github: "#", status: "ACCEPT" }
      ]
    }
  ]);

  // 3. Update openModal to accept the specific team data
  ...
  const closeModal = () => {
    setModalConfig({ isOpen: false, actionType: null, teamId: null, teamName: "" });
  };

  const handleConfirmAction = async () => {
    try {
      // You now have access to modalConfig.teamId to send to your backend!
  ...
  return (
    <div className={`py-5 position-relative min-vh-100 ${styles.pageBackground}`}>
      <div className="container" style={{ maxWidth: '900px' }}>
        
        {/* Page Title */}
        <div className="mb-5 text-center">
          <h2 className="fw-bolder" style={{ color: 'var(--color-very-dark-purple)', fontSize: 'var(--fs-h1)' }}>
            My Teams Progress
          </h2>
          <p style={{ color: 'var(--color-dark-gray)', fontSize: 'var(--fs-regular)' }}>
            Track your team members' invitation statuses and finalize your squads.
          </p>
        </div>

        {/* 4. Map over the teams array */}
        {teams.length > 0 ? (
          teams.map((team) => (
            <div 
              key={team.id} 
          ...
          <div className="text-center py-5">
            <p style={{ color: 'var(--color-gray)', fontSize: 'var(--fs-regular)' }}>
