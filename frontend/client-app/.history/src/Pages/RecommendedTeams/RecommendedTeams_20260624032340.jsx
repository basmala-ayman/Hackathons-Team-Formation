...
function RecommendedTeams() {
  ...
  const handleAccept = async ({ isOwner, recommendationId, invitationId }) => {
    ...
    if (loadingId === idToLoad) return;

    try {
      setLoadingId(idToLoad);
          ...
          displayedTeams.map((team) => {
                    ...
                    {recs.map((rec, index) => (
                      <div key={rec.id}>
                        ...
                        <TeamCard
                          ...
                          isLoading={false}
                          onAccept={() => handleAccept({ isOwner: true, recommendationId: rec.id })}
                          onReject={() => handleReject({ isOwner: true, recommendationId: rec.id })}
                        />
                      </div>
                    ))}
                  </div>
            ...
            return (
              <TeamCard
                ...
                isLoading={false}
                onAccept={() => handleAccept({ isOwner: false, invitationId: team.teamId })} // Use correct ID for invitation
