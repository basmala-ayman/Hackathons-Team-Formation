...
const { id } = useParams();
  ...
  if (!teamData) {
    return <div className="container py-5 text-center fs-3">Team not found!</div>;
  }

  const isOwner = teamData.ownerId === currentUserId;
  
  // Custom Dynamic Text Options
  const acceptLabel = isOwner ? "Send Invitation to the members" : "Accept to Join Team";
  const declineLabel = isOwner ? "Dismiss Recommendations" : "Decline Suggestion";

  const handleAccept = () => {
    alert(`Success: Fired accept workflow for ${teamData.teamName}`);
  };

  const handleDecline = () => {
    alert(`Dismissed: Removed ${teamData.teamName} from recommendations shelf`);
  };
    ...
    <div className={`min-vh-100 ${styles.pageBackground}`}>
      <div className="container py-5">
        ...
        <nav aria-label="breadcrumb" className="mb-4">
          <ol className="breadcrumb">
            <li className="breadcrumb-item">
              <Link to="/recommended-teams" className={`text-decoration-none ${styles.breadcrumbLink}`}>
                Recommended Teams
              </Link>
            </li>
            <li className={`breadcrumb-item active ${styles.breadcrumbActive}`} aria-current="page">
              Team Profile
            </li>
          </ol>
        </nav>
        ...
        <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-3">
          <h1 className={`fw-bolder mb-0 ${styles.teamTitle}`}>{teamData.teamName}</h1>
          <span className={`px-4 py-2 fw-semibold ${styles.matchBadge}`}>
            {teamData.matchLevel} Match
          </span>
        </div>
        
        <p className={`mb-4 ${styles.hackathonLine}`}>{teamData.hackathonName}</p>
        <p className={`mb-5 ${styles.description}`}>{teamData.description}</p>

        {/* Member Grid Section */}
        <div className="d-flex align-items-center gap-2 mb-4">
          <span className="fs-3">👥</span>
          <h4 className={`fw-bold mb-0 ${styles.sectionHeader}`}>Team Members</h4>
        </div>
        ...
        <div className="row g-4 mb-5">
          {teamData.members.map((member, index) => (
            <div key={index} className="col-md-6">
              <div className={`p-4 d-flex align-items-center gap-4 ${styles.memberCard}`}>
                ...
                <div>
                  <h5 className={`fw-bold mb-1 ${styles.memberName}`}>{member.name}</h5>
