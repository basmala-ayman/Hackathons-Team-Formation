...
function TeamProfile() {
  ...
  //get recommendation details from api
  const { teamData, loading, error } =
    useRecommendationDetails(recommendationId);
    console.log(teamData)
    
    const { user } = useAuth();
    const isOwner = teamData.targetTeam.ownerId  === user?.id;
    const currentUserInfo =
  teamData.recommendedMembers.find(
    (member) => member.userId === user?.id
  );

const invitationId =
  currentUserRecommendation?.invitationId;
