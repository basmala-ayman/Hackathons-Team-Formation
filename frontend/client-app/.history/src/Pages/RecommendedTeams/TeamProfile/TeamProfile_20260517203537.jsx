...
const { id } = useParams();
  ...
  if (!teamData) {
    return <div className="container py-5 text-center fs-2"><h4>Team not found!</h4></div>;
  }
  ...
  return (<div className="container py-4">
      ...
      <nav aria-label="breadcrumb" className="mb-4">
        <ol className="breadcrumb">
          <li className="breadcrumb-item"><Link to="/recommended-teams">Recommended Teams</Link></li>
          <li className="breadcrumb-item active" aria-current="page">Team Profile</li>
        </ol>
      </nav>
      ...
      <div className="d-flex justify-content-between align-items-start mb-2">
        ...
        <span className={styles.matchBadge}>{teamData.matchLevel} Match</span>
      </div>
      ...
      <div className="row g-4 mb-5">
        {teamData.members.map((member, index) => (
          <div key={index} className="col-md-6">
            <div className={`p-3 d-flex align-items-center gap-3 ${styles.memberCard}`}>
              <img src={member.avatar} alt={member.name} className={styles.memberAvatar} />
              <div>
                <h6 className="fw-bold mb-0">{member.name}</h6>
                <small className="text-muted d-block mb-2">{member.role}</small>
      ...
      <div className="d-flex justify-content-between align-items-center border-top pt-4">
        <div>
          ...
          <span className="fw-bold text-purple">{teamData.members.length}/{teamData.maxMembers} Members</span>
        </div>
