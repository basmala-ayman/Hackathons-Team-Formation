...
function Explore() {
  ...
  const handleFilter=useCallback((filterType , selectedValue)=>{
    setFilters((prevFilterState)=>{
      ...
      const updatedList=isValueIncluded?currentList.filter((item)=>item!==selectedValue):[...currentList, selectedValue]
     
      //return the filter but edit the filter type which its list is changed
      return{
      ...prevFilterState , [filterType]:updatedList
     }
    })
  ...
  const filteredHackathons = useMemo(() => {
    ...
    const isDateMatch =
      filters.date.length === 0 || filters.date.includes(hackathon.date); //no filter in date or the date value matches date of hackathon
    ...
    const isPrizeRangeMatch =
      ...
      return isSearchMatch && isDateMatch && isCategoriesMatch && isPrizeRangeMatch;
      ...
      <Container className="my-5">
        ...
        <Row className="mt-5">
          <Col lg={2} className="d-none d-lg-block">
            <FilterSideBar onFilterChange={handleFilter} onFilterChange={handleFilter} 
    activeFilters={filters}></FilterSideBar>
          </Col>
