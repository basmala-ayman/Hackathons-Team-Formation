    ...
    >
      ...
      {filterSections.map((section)=>(
          <div key={section.id} className="mb-4">
            ...
            <div className="form-check mb-2 d-flex align-items-center gap-2" key={option}>
              
              <input 
                className=${1styles.form-check-input}` 
                type="checkbox" 
                id={`${section.id}-${option}`} 
              />
              
              <label className="form-check-label text-secondary fs-4" htmlFor={`${section.id}-${option}`}>
                {option}
              </label>
