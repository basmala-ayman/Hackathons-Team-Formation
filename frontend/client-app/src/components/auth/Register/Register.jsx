...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      popUp.warn("One more step! Please check your email to verify your account.");
      navigate("/verify-email");
    } catch (err) {
      console.log("Error inside Register.jsx:", err);
      if (err.status === 409 || err.message?.toLowerCase().includes("User already exists")) {
  ...
  return (
    <AuthLayout>
      <div className={loginStyles.loginContainer}>
        ...
        <form onSubmit={handleRegister} noValidate className={loginStyles.formStack}>
          ...
          <div className={styles.checkboxWrapper}>
            <input className={styles.checkboxInput} type="checkbox" id="agree-check-box" onChange={(e) => setAgreed(e.target.checked)} />
          ...
          <p className={loginStyles.footerText}>
            ...
            <Link to="/login" className={loginStyles.link}>Login</Link>
          </p>
        </form>
        ...
        >
          <Modal.Header closeButton className="border-0 pb-0">
            <Modal.Title className="fw-bold px-3 pt-3">Terms & Conditions</Modal.Title>
          </Modal.Header>

          <Modal.Body className="px-4 pb-4">
            <div style={{ maxHeight: '400px', overflowY: 'auto', paddingRight: '10px' }}>
              <h4 className="mt-3">1. Introduction</h4>
              <p className="text-muted fs-5">Welcome to TeamCatalyst. By creating an account, you agree to follow our community guidelines...</p>

              <h4 className="mt-4">2. Privacy Policy</h4>
              <p className="text-muted fs-5">We value your privacy. Your data (name, email) is used only to personalize your experience and for AI-based team formation...</p>

              <h4 className="mt-4">3. User Conduct</h4>
              <p className="text-muted fs-5">You agree not to use the platform for any illegal activities or to harass other team members...</p>
            </div>
          </Modal.Body>
