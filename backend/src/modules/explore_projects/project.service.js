...
const getAllExploreProjects = async () => {
  ...
  return projects.map((p) => ({
    ...
    creatorId: p.ownerId,
    creatorRole: p.owner?.techRoles || "Unknown",
    creatorName: p.owner?.name || "Unknown",
    creatorPicture: p.owner?.profilePicture || null,
    hackathonTitle: p.team?.hackathon?.title || null,
    teamId: p.teamId,
    teamName: p.team?.name || null,
    teamStatus: p.team?.status || null,
    requiredRoles: p.team?.roles || [],
    requiredSkills: p.team?.skills?.map((s) => s.skill.name) || [],
